// The runner hero asset (built by assets/blender/runner.py). Picks and
...
export class Hero {
  ...
  private readonly screen: THREE.MeshStandardMaterial | null = null;
  private readonly screenGlow: number;
  ...
  private constructor(gltf: { scene: THREE.Object3D; animations: THREE.AnimationClip[] }) {
    ...
    this.screen = screen;
    this.screenGlow = screen ? (screen as THREE.MeshStandardMaterial).emissiveIntensity : 1;
    this.play('idle', 0);
  }
  ...
  update(w: World, dt: number): void {
    ...
    if (this.screen) {
      ...
      this.screen.emissiveIntensity = this.screenGlow * (1 - 0.97 * present);
    }
